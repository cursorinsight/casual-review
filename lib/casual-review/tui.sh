...
tui_label_split() {
  ...
  labels_re+='|Comments skipped|Directory'
  labels_re+='|Dry-run change messages|Dry-run payloads'
  labels_re+='|Dry-run responses|Duplicate change messages'
  labels_re+='|Duplicate comments|Duplicate messages|Duplicate verdicts'
  labels_re+='|Engine|Event|Failures'
  labels_re+='|File|Fix commit|Inline comments|Inline responses matched'
  labels_re+='|Labels|Location|Message|Mode|Notify|Progress|Pull request'
  labels_re+='|Repository'
